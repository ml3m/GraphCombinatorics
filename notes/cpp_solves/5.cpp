...
void nextPermutation(int n, int* p) {
    ...
    // If no such element exists, the permutation is the last one, so reverse it
    if (i < 0) {
        reverse(p, p + n);
    } else {
        ...
        // Step 4: Reverse the sequence from p[i + 1] to the end to get the next permutation
        reverse(p + i + 1, p + n);
    }
